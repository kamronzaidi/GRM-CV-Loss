...
wandb_name="GRM_no_finetune" #"GRM_seed1"
...
CUDA_VISIBLE_DEVICES=${devices} accelerate launch --num_processes ${n_gpu} --main_process_port ${main_process_port} run_grm_reward_train.py \
    ...
    --pair 'alpaca-13b,gpt-3.5-turbo' --num_human 1
...
for pair in "${pairs[@]}"; do
    ...
    cd ../rm_eval
    peft_name=$(ls -dt ../save_reward_models/GRM-Gemma2-2B-sftreg_GRM_no_finetune_len1024_lora32_0.0_datamt_bench_human_judgments/logs/*/ | head -n 1)
    CUDA_VISIBLE_DEVICES=${gpu} accelerate launch --main_process_port ${port} eval_grm.py --base_model ${base_model} --peft_name ${peft_name} \
                                            ...
                                            --task mtbench --layer_type ${layer_type} --num_layers ${num_layers} --pair ${pair} --affix ${0}
done